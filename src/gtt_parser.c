...
uint8_t gtt_process_packet(gtt_device *device, size_t packetstart)
{
	...
	for (size_t i = 0; i < device->wait_idx; i++)
	{
		if (!device->waitlist[i].Done)
		{
			...
			if (WaitingPacket)
			{
				device->waitlist[i].PacketStart = device->Parser.PacketStart;
...
uint8_t gtt_parser_process(gtt_device *device)
{
	...
	if (Res != -1)
	{
		...
		case GTT_PARSER_LENGTH_2:
			...
			break;
			if (device->Parser.PacketStart + device->Parser.Length > device->rx_buffer_size)
				device->Parser.state = GTT_PARSER_IDLE;
			break;
...
uint32_t gtt_parser_getU32(gtt_device* device, size_t index, size_t *outIndex)
{
	uint32_t data = (uint32_t)(
		(device->rx_buffer[index + 0] << 24) |
		(device->rx_buffer[index + 1] << 16) |
		(device->rx_buffer[index + 2] << 8) |
		(device->rx_buffer[index + 3])
		);
	*outIndex = index + 4;
	return data;
}
...
{
	uint16_t data = (uint16_t) (device->rx_buffer[index] << 8) | device->rx_buffer[index + 1];
	*outIndex = index + 2;
	return data;
}
...
{
	int16_t data = (int16_t)(device->rx_buffer[index] << 8) | device->rx_buffer[index + 1];
	*outIndex = index + 2;
	return data;
}
...
gtt_number gtt_parser_getNumber(gtt_device* device, size_t index, size_t *outIndex)
{
	gtt_number result = { 0 };
	result.NumberType = gtt_parser_getU8(device, index, outIndex);
	switch (result.NumberType)
	{
		case ePropertyType_U8:
			result.value_uint8 = gtt_parser_getU8(device, *outIndex, outIndex);
			break;
		case ePropertyType_U16:
			result.value_uint16 = gtt_parser_getU16(device, *outIndex, outIndex);
			break;
		case ePropertyType_S16:
			result.value_int16 = gtt_parser_getS16(device, *outIndex, outIndex);
			break;
		case ePropertyType_Float:
			result.value_float = gtt_parser_getFloat(device, *outIndex, outIndex);
			break;
	}
	return result;
}
