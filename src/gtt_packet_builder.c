...
void gtt_packetbuilder_writeNumber(gtt_device *device, gtt_number value)
{
	gtt_packetbuilder_writeU8(device, value.NumberType);
	switch (value.NumberType)
	{
		case ePropertyType_U8:
			gtt_packetbuilder_writeU8(device, value.value_uint8);
			break;
		case ePropertyType_U16:
			gtt_packetbuilder_writeU16(device, value.value_uint16);
			break;
		case ePropertyType_S16:
			gtt_packetbuilder_writeS16(device, value.value_int16);
			break;
		case ePropertyType_Float:
			gtt_packetbuilder_writeFloat(device, value.value_float);
			break;
	}
}
